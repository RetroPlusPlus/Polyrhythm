...
| A hosted machine's video (`VmConfig{.video = true}` or `video(on)`; `video()` hands the last complete frame to a `DrawLayer` as `RasterContent`, composited by z among native layers, on either clock) | available | co-execution.md / draw-state.md |
| A hosted machine's input (`gb::Button` / `snes::Button` as Actions enums, `gb::held(input)` / `snes::held(input)` pack the set your game already resolved — `snes::Ports` both sockets — `Vm::buttons` holds it — a level, landing at the machine's next step boundary on either clock) | available | co-execution.md / input.md |
| A hosted machine's battery save (`VmConfig{.key}` names the machine, `batterySave` names the file per cartridge, the core's own extension always ends it; read back when an image is hosted, written after the guest changes it, off every paced thread — the game calls nothing) | available | vm-and-routines.md |
| A hosted cartridge's sound (`enableAudio(rate, onSample)` — each stereo frame the machine's sound chip finishes, at `rate`, on the thread that steps it; the game owns the hand-off to an `AudioSink`) | available | co-execution.md |
| The SNES core (`Vm::SNES` / `VMPlatform::Snes` — Snaggletooth behind the same seam: a cartridge hosted, booted, paced on either clock, drawn, heard, played on both ports and its battery save kept; the naming, escape, watch and call verbs refuse with `std::logic_error`) | available | co-execution.md / vm-and-routines.md |
