...
```cpp
Vm::SNES a;                                                          // platform and timing pre-bound
Vm::SNES b{VmConfig{.key = "adventure", .video = true}};             // the same, with its key and outputs declared
Vm       c{VMPlatform::Snes, VmConfig{.key = "adventure", .video = true}};  // the platform named at construction
```

`Vm::GB`, `Vm::GBC` and `Vm::SNES` are `Vm`s with the console fixed at the type — they add no state, so
one goes anywhere a `Vm` is expected. `VmConfig` is the same aggregate on every console: `.key` names
the machine for keeping its guest's save, `.video` declares that it draws. `vm.platform()` reports the
console back. Constructing a `Vm` for
