...
```cpp
Vm::GB  dmg{VmConfig{.key = "adventure", .video = true}};  // VMPlatform::GameBoy,      TimingProfile::GameBoy
...
`Vm`s with the console fixed at the type, each taking the same `VmConfig` or none;
`Vm{VMPlatform::GameBoyColor, VmConfig{…}}` is the same machine with the console named at construction.
